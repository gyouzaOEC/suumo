...
def index(request):
    ...
    '賃貸アパート': np.bool_(1 if 'S' in initial_data['sldk'] else 0),
    '賃貸テラス・タウンハウス': np.bool_(1 if 'S' in initial_data['sldk'] else 0),
    '賃貸マンション': np.bool_(1 if 'S' in initial_data['sldk'] else 0),
    '賃貸一戸建て': np.bool_(1 if 'S' in initial_data['sldk'] else 0),
})
